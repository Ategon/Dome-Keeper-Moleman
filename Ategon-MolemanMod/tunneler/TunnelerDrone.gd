...
var TUNNEL = preload("res://mods-unpacked/Ategon-MolemanMod/tunnel/Tunnel.tscn")
...
func generateMiningDrops(tile, tileCoord):
	...
	for i in range(0, drops):
		curMiningDrops.append({
			"damage": ((i+1) / (drops)) * Data.of("keepermole.tunnelMiningDropHealthLossPercent") * tile.max_health,
			"type": tile.type
		})
...
func _physics_process(delta):
	...
	if is_instance_valid(tile) and tile.type == CONST.BORDER:
		translate(-dir * Data.of("keepermole.tunnelerTravelSpeed") * delta * 2)
	...
	# Reached target?
	if global_position.distance_to(targetPos) <= delta*Data.of("keepermole.tunnelerTravelSpeed"):

		# Drill until completion
		if is_instance_valid(tile) and tile.has_meta("destructable") and tile.get_meta("destructable"):

			if curDmgWaitTime <= 0:
				curDmgWaitTime = Data.of("keepermole.tunnelerTargetDamageFrequency")
				tile.hit(-drillDir, Data.of("keepermole.tunnelerTargetDamage"))
		...
		# Moving
		var isMining = (tileCoord != targetTileCoord and Data.ofOr("keepermole.tunnelMining", false) and is_instance_valid(tile)
			...
			if is_instance_valid(tile) and tile.has_meta("destructable") and tile.get_meta("destructable"):
				...
				if tileCoord != targetTileCoord:
					...
					if curSoundWaitTime <= 0 and not isMining:
						curSoundWaitTime = Data.of("keepermole.tunnelerTravelSoundFrequency")
						$TunnelDrillSound.play()
					else:
						curSoundWaitTime -= delta

					if curDmgWaitTime <= 0:
						curDmgWaitTime = Data.of("keepermole.tunnelerMiningTravelDamageFrequency") if isMining else Data.of("keepermole.tunnelerTravelDamageFrequency")

						tile.hit(-drillDir, Data.of("keepermole.tunnelerMiningTravelDamage") if isMining else Data.of("keepermole.tunnelerTravelDamage"))
		...
		translate(dir * (Data.of("keepermole.tunnelerMiningTravelSpeed") if isMining else Data.of("keepermole.tunnelerTravelSpeed")) * delta)
